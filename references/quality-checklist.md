...
- **Critical:** Give every core-concept heading one logo-only Baidu button and one logo-only Google button. For a `中文概念（English term）` heading, the decoded Baidu query must exactly equal the Chinese text before the parentheses and the decoded Google query must exactly equal the English text inside them. Allow the same unchanged query only when a proper noun or abbreviation has no parenthetical bilingual split. Never append context, definitions, or related topics.
