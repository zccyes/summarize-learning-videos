...
7. Give each core concept its own plain-text heading. Immediately below it, run `scripts/render_search_buttons.py --term "<exact concept name>"` to add one logo-only Baidu button and one logo-only Google button. Search exactly the visible concept name before any parenthetical translation; never append explanatory, disambiguating, topical, or translated keywords. Never use an unresolved wikilink as the concept heading.
