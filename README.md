...
- 保留开头的机器可读 YAML，同时在笔记末尾生成面向阅读者的属性表，避免元数据抢占正文注意力。
- 为每个核心概念生成只显示百度与 Google Logo 的一键搜索按钮；百度只搜索括号前的中文概念，Google 只搜索括号内的英文术语，不附加解释性关键词，无需安装 Obsidian 插件。
- 维护视频学习索引。
- 保存前执行语义质量复核和结构化质量门。
...
3. 带原视频跳转链接的时间轴。
4. 分章节详细提炼。
5. 核心概念与术语，以及百度、Google Logo 搜索按钮。
6. 关键论证、案例和证据。
7. 可复用的方法、流程或分析框架。
8. 作者的假设、限制和可能争议。
9. 可执行清单。
10. 复习问题。
11. 用于关系图谱聚合的规范化关键词标签，以及确实存在的 Obsidian 笔记链接。
12. 提炼质量和不确定性说明。
13. 位于全文最后的笔记属性表。

为了同时保留 Obsidian 的属性检索能力和更专注的阅读顺序，Markdown 文件开头仍会保存 YAML frontmatter，但 SLV 会在末尾自动生成一份可读属性表。建议在 Obsidian 中选择 **设置 → 编辑器 → 文档中的属性 → 隐藏**。这样顶部属性面板不会显示，正文会直接从标题和来源说明开始；YAML 仍然正常支持标签、别名、搜索和关系图谱。
